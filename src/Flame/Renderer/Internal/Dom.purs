...
foreign import start_ ∷ ∀ message. EffectFn4 (MessageWrapper message) DomNode (Maybe message → Effect Unit) (Html message) RenderingState
foreign import startFrom_ ∷ ∀ message. EffectFn4 (MessageWrapper message) DomNode (Maybe message → Effect Unit) (Html message) RenderingState
foreign import resume_ ∷ ∀ message. EffectFn2 RenderingState (Html message) Unit

-- | Mounts the application on a DOM node
-- |
-- | The node will be set as the parent and otherwise unmodified
start ∷ ∀ message. DomNode → (message → Effect Unit) → Html message → Effect RenderingState
start parent updater = EU.runEffectFn4 start_ Just parent (maybeUpdater updater)

-- | Hydrates a server-side rendered application
startFrom ∷ ∀ message. DomNode → (message → Effect Unit) → Html message → Effect RenderingState
...
resume ∷ ∀ message. RenderingState → Html message → Effect Unit
